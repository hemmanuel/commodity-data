...
for year in years:
    try:
        # URL pattern changes slightly?
        # 2006-2016 history file: fut_disagg_txt_hist_2006_2016.zip
        # Annual files: fut_disagg_txt_YYYY.zip
        
        if year == 2006:
            # Download the history file once
            url = f"{base_url}/fut_disagg_txt_hist_2006_2016.zip"
            print(f"Downloading history file (2006-2016): {url}...")
        elif year <= 2016:
            continue # Already covered by history file
        else:
        ...
        # Also maybe "CRUDE OIL, LIGHT SWEET - ICE FUTURES EUROPE" (Brent?)
        # Let's stick to WTI for now as requested ("Crude Oil")
        
        # Normalize column names just in case
        df.columns = [c.strip() for c in df.columns]
        
        target_market = "CRUDE OIL, LIGHT SWEET - NEW YORK MERCANTILE EXCHANGE"
        mask = df['Market_and_Exchange_Names'].str.strip() == target_market
        df_filtered = df[mask].copy()
        
        if df_filtered.empty:
            print(f"No data found for {target_market} in {year}")
            continue
        ...
        # Check if columns exist (some years might differ)
