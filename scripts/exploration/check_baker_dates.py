...
try:
    ...
    # Skip first 9 rows to get to header
    df = pd.read_excel(xlsx_path, sheet_name='NAM Weekly', skiprows=9, engine='openpyxl')
    
    print("Columns:", df.columns.tolist())
    
    if 'US_PublishDate' in df.columns:
        df['US_PublishDate'] = pd.to_datetime(df['US_PublishDate'])
