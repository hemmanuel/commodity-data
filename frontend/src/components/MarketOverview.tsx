...
const MarketOverview = ({ category }: { category: string }) => {
  ...
  const fetchSeries = async (isLoadMore = false, isReset = false) => {
    ...
    try {
      ...
      const res = await axios.get(`http://localhost:8000/market/search?${params.toString()}`);
    ...
    <div className="flex h-full gap-6">
      ...
      <div className="w-1/3 flex flex-col space-y-4">
        <form onSubmit={handleSearch} className="relative">
          <input
            type="text"
            placeholder={`Search ${category} data...`}
